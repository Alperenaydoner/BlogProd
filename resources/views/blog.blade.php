                            ...
                            <div class="row">
                                <div class="sidebar-wrapper col-4">
                                    <div class="profile-container">
                                        <img class="profile box-image-preview img-fluid img-circle elevation-2" src="{{ isset($user['image_path']) && !empty($user['image_path']) ? asset('assets/images/'. $user['image_path'])  : asset('assets/images/user-thumb.jpg') }}"
                                        ...
                                        <ul class="list-unstyled contact-list">
                                            @if (!empty($user['email']))
                                                <li class="email"><i class="fa-solid fa-envelope"></i>
                                                    <a class="ms-3"
                                                        href="mailto: {{ isset($user['email']) ? $user['email'] : 'yourmail@example.com' }}">{{ isset($user['email']) ? $user['email'] : '' }}</a>
                                                </li>
                                            @endif
                                            @if (!empty($user['phone_number']))
                                                <li class="phone"><i class="fa-solid fa-phone"></i>
                                                    <a class="ms-3"
                                                        href="tel:{{ isset($user['phone_number']) ? $user['phone_number'] : '' }}">{{ isset($user['phone_number']) ? $user['phone_number'] : '' }}</a>
                                                </li>
                                            @endif
                                            @if (!empty($user['website']))
                                                <li class="website"><i class="fa-solid fa-globe"></i>
                                                    <a class="ms-3"
                                                        href="{{ isset($user['website']) ? $user['website'] : '' }}"
                                                        target="_blank">{{ isset($user['website']) ? $user['website'] : '' }}</a>
                                                </li>
                                            @endif
                                            @if (!empty($user['linkedin_link']))
                                                <li class="linkedin"><i class="fa-brands fa-linkedin-in"></i>
                                                    <a class="ms-3"
                                                        href="{{ isset($user['linkedin_link']) ? $user['linkedin_link'] : '' }}"
                                                        target="_blank">{{ isset($user['linkedin_link']) ? $user['linkedin_link'] : '' }}</a>
                                                </li>
                                            @endif
                                            @if (!empty($user['github_link']))
                                                <li class="github"><i class="fa-brands fa-github"></i>
                                                    <a class="ms-3"
                                                        href="{{ isset($user['github_link']) ? $user['github_link'] : '' }}"
                                                        target="_blank">{{ isset($user['github_link']) ? $user['github_link'] : '' }}</a>
                                                </li>
                                            @endif
                                            @if (!empty($user['twitter']))
                                                <li class="twitter"><i class="fa-brands fa-twitter"></i>
                                                    <a class="ms-3"
                                                        href="{{ isset($user['twitter']) ? $user['twitter'] : '' }}"
                                                        target="_blank">{{ isset($user['twitter']) ? $user['twitter'] : '' }}</a>
                                                </li>
                                            @endif
                                        </ul>
                                    </div>
                                        ...
                                        <div class="education-container container-block">
                                            ...
                                            @foreach ($user['education'] as $education)
                                                <div class="item">
                                                    ...
                                                    <div class="time">
                                                        {{ isset($education['edu_start_date']) ? $education['edu_start_date'] : '' }}
                                                        -
                                                        {{ isset($education['edu_end_date']) ? $education['edu_end_date'] : '' }}
                                                    </div>
                                                    <p>{{ isset($education['education_description']) ? $education['education_description'] : '' }}
                                                    </p>
                                                </div>
                                            @endforeach
                                            <!--//item-->
                                            ...
                                            @foreach ($user['projects'] as $projects)
                                                <div class="item">
                                                    <span class="project-title">
                                                        <a
                                                            target="_blank">{{ isset($projects['project_title']) ? $projects['project_title'] : '' }}</a>
                                                    </span>
                                                    -
                                                    ...
                                                    <div class="item">
                                                        ...
                                                        <div class="progress level-bar">
                                                            <div class="progress-bar theme-progress-bar"
                                                                role="progressbar"
                                                                style="width: {{ isset($skills['skill_percentage']) ? $skills['skill_percentage'] : '0' }}%"
                                                                aria-valuenow="{{ isset($skills['skill_percentage']) ? $skills['skill_percentage'] : '0' }}"
